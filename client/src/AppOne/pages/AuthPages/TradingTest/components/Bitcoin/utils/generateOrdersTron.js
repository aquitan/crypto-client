const randomizeOrdersTron = (min, max) => {
    let val = Number(((Math.random() < 0.49 ? 1 : 1.85) * Math.random() * (min + max) * 10).toFixed(2));
  ...
  export const generateOrdersTron = (rate) => {
    let arr = []
    for (let i = 0; i < 15; i++) {
      let p = Number(rate) + randomizeOrdersTron(0.12, 0.89);
